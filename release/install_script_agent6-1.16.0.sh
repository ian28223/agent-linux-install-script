...
echo -e "\033[34m\n* Datadog Agent 6 install script v${install_script_version}\n\033[0m"
...
if [ -n "$fips_mode" ]; then
    ...
    if [[ -n "$agent_minor_version_without_patch" ]] && [[ "${agent_minor_version_without_patch}" -lt 41 ]]; then
        ERROR_MESSAGE="FIPS mode is only available since version 6.41.0 and requested minor version is $agent_minor_version"
        ERROR_CODE=$INVALID_PARAMETERS_CODE
        printf "\033[31m$ERROR_MESSAGE\033[0m\n"
        report_telemetry
        exit 1;
...
if [ -n "$system_probe_ensure_config" ]; then
  if [ -e "$system_probe_config_file" ] && [ -z "$upgrade" ]; then
    printf "\033[34m\n* Keeping old $system_probe_config_file configuration file\n\033[0m\n"
  else
    if [ ! -e "$system_probe_config_file" ]; then
      $sudo_cmd cp "$system_probe_config_file.example" "$system_probe_config_file"
    fi

    $sudo_cmd chown root:dd-agent "$system_probe_config_file"
    $sudo_cmd chmod 640 "$system_probe_config_file"
  fi
fi
...
install_method:
  tool: install_script
  tool_version: install_script_agent6
